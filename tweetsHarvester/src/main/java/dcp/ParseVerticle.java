...
/**
 * This verticle is responsible for parsing the street map data. It divides the
 *   road into blocks and generates location info for each block and sends this
 *   info to Query Verticle.
 */
public class ParseVerticle extends Verticle {
        ...
        for (int i = 0; i < rowDataSize; ++i) {
            ...
            if (maxDistance <= radius * 2000) { // compare with search diameter in meter
                ...
                final JsonObject queryInfo = 
                        queryInfoBuilder(tag, streetName, queryLat, queryLon, radius);
        ...
        if (blocks.size() > 0) {
            ...
            vertx.setPeriodic(interval, new Handler<Long>() {
                public void handle(Long timerID) {
                    log.info(String.format("CurrentBlock: %d, TotalBlocks: %d", 
                            blockCount, totalBlocks));
                    final JsonObject queryInfo = blocks.get(blockCount);
                    eventBus.send(Constants.QUEUE_QUERYINFO, queryInfo, 
                        ...
                        if (++blockCount == totalBlocks) {
                            blockCount = 0;
                            log.info(String.format("The whole road data is "
                                    + "traversed %d time(s).", ++roundCount));
                        }
                }
            });
    ...
    private static JsonObject queryInfoBuilder(String tag, String streetName, 
