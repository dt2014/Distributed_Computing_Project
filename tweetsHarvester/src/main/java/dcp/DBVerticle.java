...
/**
 * This verticle is responsible to communicate with CouchDB databases. 
 * It basically stores the tweets in the database.
 */
public class DBVerticle extends Verticle {
    
    private int dbInsertFailCounter = 0;
    private int NumOfDBFailForLog = 100;
    ...
    public void start() {
        ...
        evenBus.registerHandler(Constants.QUEUE_TWDATA, 
                new Handler<Message<JsonObject>>() {
            @Override
            public void handle(Message<JsonObject> message) {
                final JsonObject tweet = message.body();
                final String id = tweet.getString("id_str");
                HttpClientRequest request = client.request("PUT", 
                        config.getString("dbName") + id, 
...
//                              log.error(String.format("Over %d conflicts in "
//                                      + "inserting, seems no more new tweets "
//                                      + "so far...\nSystem exiting.", dbConflictCount));
//                              container.exit();
//                          }
                            if (dbInsertFailCounter > NumOfDBFailForLog) {
                                log.warn(String.format("Over %d inserting conflicts so far.", 
                                        NumOfDBFailForLog));
                                NumOfDBFailForLog += 100;
                            }
                        }
                    }
